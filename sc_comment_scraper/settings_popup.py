...
class SettingsPopUp(Config):
    ...
    def __init__(self, master, pd, settings):
        ...
        #wait time
        self.set_wt_label = ttk.Label(self.set_popup,
                                      text='Wait time  = ',
                                      padding=[pd*3, pd, 0, pd],
                                      style='popup.small.TLabel')
        self.set_wt_label.grid(row=0, column=0)
        self.set_wt = tk.StringVar(value=0.5)
        self.set_wt_entry = ttk.Entry(self.set_popup, 
                                      width=5, 
                                      textvariable=self.set_wt)
        self.set_wt_entry.grid(row=0, 
                               column=1)
        ...
        self.set_ovr_label = ttk.Label(self.set_popup,
                                       text='Scroll parameter  = ',
                                       padding=[pd*3, pd, 0, pd],
                                       style='popup.small.TLabel')
        self.set_ovr_label.grid(row=1, column=0)
        self.set_ovr = tk.StringVar(value=settings.scroll)
        self.set_ovr_entry = ttk.Entry(self.set_popup, 
                                       width=5,
                                       textvariable=self.set_ovr)
        self.set_ovr_entry.grid(row=1, 
                                column=1)
