...
class UrlFrame(Config):
    ...
    def __init__(self, scr_frame, pd, url_input):
        ...
        self.style.configure('tick.TLabel', 
                             foreground='#43eb34', 
                             background=self.tab_colour, 
                             font=('Arial', '13', 'bold'))
        self.style.configure('cross.TLabel', 
                             foreground='Red', 
                             background=self.tab_colour, 
                             font=('Arial', '13'))
        
        #url input heading 1
        self.url_heading_label = ttk.Label(self.url_frame, 
                                           text='URL Input Method', 
                                           style='sub.TLabel', 
                                           padding=[pd, pd, pd, pd])
        self.url_heading_label.grid(row=0, 
                                    column=0, 
                                    columnspan=3, 
                                    sticky='nw')
        ...
        def browse_files():
            ...
            if filename:
                self.url_subheading_label.configure(text="File Opened: "+filename)
        ...
        def textinp():
            ...
            self.url_subheading_label.grid(row=5, 
                                           column=0, 
                                           columnspan=3, 
                                           sticky='w')
            ...
            self.url_browse_button.grid(row=6, 
                                        column=0, 
                                        columnspan=3, 
                                        sticky='w', 
                                        padx=pd*3)
        ...
        def manualinp():
            ...
            def update_urls():
                ...
                def add_to_url_list(var, row): #check these arguments
                    url_input.url_list = [text.get() for row, text in self.url_variables_dict.items() if text.get().strip()]

                for row, var in self.url_variables_dict.items():
                    var.trace_add('write', lambda name, index, mode, var=var, row=row: add_to_url_list(var, row))
            
            #creates a new entry widget and moves add button
            def new_url():
                self.url_entry_dict = {}
                r = self.url_add_button.grid_info()['row']
                self.url_add_button.grid(row=r+1, 
                                         column=0, 
                                         padx=pd*3, 
                                         pady=pd, 
                                         sticky='w')
                ...
                self.url_entry_dict[r].grid(row=r,
                                            column=0,
                                            columnspan=3,
                                            sticky='w',
                                            padx=pd*3,
                                            pady=pd)
                self.check_entry(text=self.url_variables_dict[r],
                                 row=r,
                                 column=3,
                                 mode='track')
                update_urls()
            ...
            self.url_subheading_label = ttk.Label(self.url_frame, 
            ...
            self.url1_entry = ttk.Entry(self.url_frame, 
                                        width=70, 
                                        textvariable=self.url1_value)
            self.widget_list.append(self.url1_entry)
            self.url1_entry.grid(row=6, 
                                 column=0, 
                                 columnspan=3, 
                                 sticky='w', 
                                 padx=pd*3, 
            ...
            self.url_add_button = ttk.Button(self.url_frame, 
                                    text='+', 
                                    style='add.TButton', 
                                    width=2, 
                                    command=new_url)
            self.widget_list.append(self.url_add_button)
            self.url_add_button.grid(row=7, 
                                     column=0, 
                                     padx=pd*3, 
                                     pady=pd, 
                                     sticky='w')
            ...
            self.check_entry(text=self.url1_value,   
                             mode='track')
        ...
        self.url_rb1 = ttk.Radiobutton(self.url_frame, 
                                       text='Manual', 
        ...
        self.url_rb2 = ttk.Radiobutton(self.url_frame, 
                                       text='From .txt file', 
                                       padding=[pd*3, pd, pd, 0], 
                                       value='L', 
                                       variable=self.retrieval_type, 
        ...
        self.url_rb3 = ttk.Radiobutton(self.url_frame, 
        ...
        self.url_rb4 = ttk.Radiobutton(self.url_frame, 
                                       text='From playlist', 
                                       padding=[pd*3, pd, pd, pd], 
                                       value='P', 
                                       variable=self.retrieval_type,    
        ...
        self.url_max_label = ttk.Label(self.url_frame, 
                                       text='Enter max urls  =', 
                                       style='small.TLabel', 
                                       padding=[pd*3, pd, pd, pd])
        self.url_max_label.grid(row=3, column=0)
        self.url_max_entry = ttk.Entry(self.url_frame, 
                                       width=5, 
                                       textvariable=self.url_max)
        self.url_max.trace_add("write", url_max_trace)
        self.url_max_entry.grid(row=3, column=1, sticky='w')

        #subheading 2
        self.url_heading2_label = ttk.Label(self.url_frame, 
                                            text='URL Entry', 
                                            style='sub.TLabel', 
                                            padding=[pd, pd*3, pd, pd])
        self.url_heading2_label.grid(row=4, 
                                     column=0, 
                                     columnspan=3, 
                                     sticky='w')
    ...
    def check_entry(self, mode, text=None, row=6, column=3):
        ...
        def tick_or_cross(*args):
            ...
            if condition1 and condition2 in text.get():                    
                ticks_crosses[row] = ttk.Label(self.url_frame, 
                                               text='✓', 
                                               style='tick.TLabel',
                                               width=2)
            elif not text.get():
