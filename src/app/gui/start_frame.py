...
class StartFrame(Config):
    def __init__(self, scr_frame, pd, master, url_input, settings, filters):
        ...
        def test():
            ...
            self.scraper.test(
                settings=settings, url_input=url_input, filters=filters
            )
        ...
        self.style.map(
            "start.TButton", foreground=[("active", "!disabled", "black")]
        )
        ...
        self.scraper = Scraper(
            settings=settings, start_frame=self.start_frame, pd=pd
        )
