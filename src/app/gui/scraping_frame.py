...
class ScrapingFrame(Config):
    ...
    def __init__(self, notebook, pd, master, url_input, settings, filters):
        ...
        self.scr_frame = ttk.Frame(notebook, 
                                   style='TFrame')
        self.scr_frame.pack(fill='both', expand=True)

        self.url_frame = UrlFrame(self.scr_frame, 
                                  pd=pd, 
                                  url_input=url_input).url_frame
        self.set_frame = SettingsFrame(self.scr_frame, 
                                            pd=pd, 
                                            master=master, 
                                            settings=settings, 
                                            filters=filters).set_frame

        #will this pass the updated filters and settings etc.?
        self.start_frame = StartFrame(self.scr_frame, 
                                      pd=pd, 
                                      master=master, 
                                      url_input=url_input, 
                                      settings=settings, 
                                      filters=filters).start_frame
