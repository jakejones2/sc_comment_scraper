...
class SettingsFrame(Config):
    def __init__(self, scr_frame, pd, master, settings, filters):
        ...
        self.set_heading_label = ttk.Label(
            self.set_frame,
            text="Scraping Settings",
            style="sub2.TLabel",
            padding=[pd, pd, pd, pd],
        )
        self.set_heading_label.grid(row=0, column=0, columnspan=5, sticky="nw")
        self.widget_list = []
        ...
        self.set_maxnum_entry = ttk.Entry(
            self.set_frame, width=7, textvariable=self.set_maxnum_var
        )
        self.set_maxnum_entry.grid(row=2, column=2, columnspan=3, sticky="w")
        ...
        # advanced settings
        def adv_settings():
            self.set_adv_pop = SettingsPopUp(
                master=master, pd=pd, settings=settings
            )

        self.set_adv_button = ttk.Button(
            self.set_frame,
            text=" Advanced Settings ",
            command=adv_settings,
            style="internal2.TButton",
        )
        self.set_adv_button.grid(
